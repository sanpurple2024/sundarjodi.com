...
//========================================================================// 
    public function new_registration_post() {
        ...
        $signupby = trim($this->post('signupBy'));
        ...
        // Validate the post data
        if(!empty($first_name) && !empty($phone) && !empty($email) && !empty($password)){
...
//========================================================================//   
    public function resendotp_post() {
        // Get the post data
    $phone_num = trim($this->post('mobile'));
    $otp = rand(10000,99999);
...
//========================================================================//   
 ...
 foreach ($query12->result() as $row1){
  ...
  //========================================================================// 
    ...
    public function new_basic_info_post() {
        
        $profile_id = trim($this->post('profile'));
 ...
 foreach ($queryha->result() as $rowha){
        $habbitinfoid =	$rowha->id;
               ...
               // $personalData['religion'] = $religion;
                $personalData['dob'] = $dob;
                $personalData['martial_status'] = $marital_status;
               // $personalData['caste'] = $caste;
    ...
    public function new_about_me_post() {
        
        $profile_id = trim($this->post('profile'));
        $about_me = trim($this->post('about_me'));
        $watsapp_num = trim($this->post('watsapp_num'));
            ...
            $userid =	$row1->id;
                ...
                $personalData['watsapp_num'] = $watsapp_num;
             $personalDataupdate = $this->user->personal_data_update($personalData, $userid);   
                if($personalDataupdate){
...
//========================================================================// 
    ...
    public function new_family_info_post() {
        ...
        $religion = trim($this->post('religion'));
        $caste = trim($this->post('caste'));
        ...
        $this->db->where('profile',$profile_id);
            $query1 = $this->db->get('user_register');
        ...
        $contactinfoid =	$rowc->id;
          ...
          $contactDataupdate = $this->user->contact_data_update($contactData, $contactinfoid);
           ...
           $personal = array();
             ...
             $personalda = $this->user->personal_data_update($personal, $registerid); 
...
//========================================================================// 
    ...
    public function personal_information_post() {
        ...
        $profile_created_for = trim($this->post('profile_created_for'));
        $dob = trim($this->post('dob'));
        $birth_time = trim($this->post('birth_time'));
 ...
 foreach ($queryha->result() as $rowha){
    ...
    public function basic_inform_post() {
 ...
 foreach ($queryha->result() as $rowha){
   ...
   //========================================================================// 
    ...
    public function family_information_post() {
        ...
        $profile_id = trim($this->post('profile'));
        $fatherrname = trim($this->post('fatherrname'));
        ...
        $motherrname = trim($this->post('motherrname'));
 ...
 foreach ($query12->result() as $row1){
    ...
    public function family_inform_post() {
...
public function education_work_post() {
 ...
 foreach ($query12->result() as $row1){
    ...
    //=====================================================================//
public function edu_wrk_inform_post() {
        ...
        $about_career = trim($this->post('about_career'));
 ...
 foreach ($query12->result() as $row1){
       ...
       if(!empty($profile_id)){
                ...
                $personalData['about_career'] = $about_career;
...
/**********************************************************************/
     public function districts_post(){   
         $state_id  = trim($this->post('state_id')); 
        $this->db->select('*');
...
/**********************************************************************/
 ...
 //================================================================//
  ...
  {
    $doc_type  = trim($this->post('doc_type'));
    $profile_id  = trim($this->post('profile'));
     $this->db->where('profile',$profile_id);
    $query12 = $this->db->get('user_register');
    if($query12->num_rows()>0){
...
// $mem = $years.' years, '.$months.' months'; 
  ...
  //================================================================//
    ...
    $logged_user_profile  = trim($this->post('logged_user_profile'));
    ...
    $log_user_dob =	$row13->dob;
    $diffl = date_diff(date_create($log_user_dob), date_create($today));
	       $log_user_user_age = $diffl->format('%y');
    $gender =	$row13->gender;
    $usermartial_status =	$row13->martial_status;
    } }
    if($gender == 'F'){
    $show_gender = 'M';
    }else{
    $show_gender = 'F';
     ...
     if($show_gender == 'F'){
                $diff1 = date_diff(date_create($log_user_dob), date_create($today));
	                  $age1 = $diff1->format('%y') - 5;   ;
	             }else if($show_gender == 'M'){
                $diff1 = date_diff(date_create($log_user_dob), date_create($today));
	                  $age1 = $diff1->format('%y') + 5;   ; 
	              }    
...
$query = $this->db->get();
 ...
 $countnwmatc = $this->db->query("select id from  user_register where gender = '$show_gender' AND  caste = '$user_caste' AND  status = '1' AND TIMESTAMPDIFF(YEAR, dob, CURDATE())  >= '$log_user_user_age' AND TIMESTAMPDIFF(YEAR, dob, CURDATE())  >= '$age1' ");
...
if($city_name!=""){
    $this->db->where('contact_info.perm_city', $city_name); 
}
...
if($log_user_marital_status_pe != ''){
            $maritalstatus = explode(", ",$log_user_marital_status_pe);
			if($log_user_marital_status_pe == '' || $log_user_marital_status_pe == 'Any'){
			    $marital_status = '';
			}else{
			...
			if($maritalstatus1 == '""'){
			   ...
			   $marital_status = 'martial_status IN('.$maritalstatus1.')'; 
			   $this->db->where($marital_status); 
			}
			}
}else{
    $this->db->where('user_register.martial_status',$usermartial_status); 
}

if($log_user_height_from_pe != '' && $log_user_height_to_pe != ''){
$height = "height >= '$log_user_height_from_pe'
AND height <= '$log_user_height_to_pe'";
$this->db->where($height); 
}
if($log_user_age_from_pe != '' && $log_user_age_to_pe != ''){
 $age = "TIMESTAMPDIFF(YEAR, dob, CURDATE()) >= '$log_user_age_from_pe'
AND TIMESTAMPDIFF(YEAR, dob, CURDATE()) <= '$log_user_age_to_pe'";
$this->db->where($age); 
			}
$this->db->where('user_register.gender', $show_gender); 
...
$query1 = $this->db->get();
  $newmatches = $query1->result_array();
				 ...
				 $this->response([
                    'status' => TRUE,
                    'message' => 'New matches Fetch Successfully',
                    'tcount' => (!empty($newmatches)) ? count($newmatches) : "0",
                    'profile' => $newmatches,
...
$query1 = $this->db->get();
  $matchofday = $query1->result_array();
   ...
   $countmatch = $this->db->query("select * from  user_register where gender = '$show_gender' AND caste = '$user_caste' AND status = '1' AND $height AND $age AND martial_status = '$usermartial_status' group by profile ");
...
$query1 = $this->db->get();
  $matchofday = $query1->result_array();
    ...
    $countloo = $this->db->query("select * from  user_register where gender = '$show_gender' AND caste = '$user_caste' AND status = '1' AND $height AND $age AND martial_status = '$usermartial_status' group by profile");
...
$query1 = $this->db->get();
  $maritalstatus = $query1->result_array();
    ...
    if($category == 'cities' || $category == 'highest_education' || $category == 'occupation'){
        ...
        $splittedcity = explode(", ", $city_name);
       $splittedhigedu = explode(", ", $high_education);
       $splittedocup = explode(", ", $occup_name);
...
if($category == 'cities'){
if($city_name == 'Any'){ }else{
$this->db->where_in('contact_info.perm_city', $splittedcity); 
}
}
if($category == 'highest_education'){
    if($high_education == 'Any'){ }else{
$this->db->where_in('education_work.highest_education', $splittedhigedu); 
}
}
if($category == 'occupation'){
     if($occup_name == 'Any'){ }else{
$this->db->where_in('education_work.occup', $splittedocup); 
}
}
...
$this->db->where('user_register.status', 1); 
$this->db->order_by('user_register.id','desc'); 
$this->db->group_by('profile_images.reg_id'); 
 if ($offset == 0) {
        $this->db->limit($limit);
    } else if ($offset != 0) {
        $this->db->limit($limit, $offset);
...
// print_r($this->db->last_query());  
  ...
  $this->db->select('COUNT(user_register.id) AS tcount,user_register.id,user_register.verified,user_register.created_user,user_register.first_name,user_register.profile,user_register.caste,user_register.dob,user_register.height,user_register.martial_status,user_register.status,contact_info.perm_city,profile_images.file_name,education_work.occup,education_work.education_field,memberships.payment_mode,interest.sent,favourites.user_logged_id as favourite');
$this->db->from('favourites');
...
$query2 = $this->db->get();
  $interest_received = $query2->result_array();
   ...
   function view_contact_post() {
    ...
    $log_user_profile  = trim($this->post('logged_user_profile'));
    $profile  = trim($this->post('view_user_profile'));
           ...
           $this->db->select("*");
                   ...
                   if ($queryr->num_rows() > 0){
                      foreach($queryr->result() as $rowi)
                       ...
                       $this->db->set('contact_viewed_date', date('Y-m-d'));
...
if($check){
 ...
 $notifications =   $this->db->insert('notifications', $data_notify); 
  ...
  //================================================================//
    ...
    $logged_user_profileid = trim($this->post('logged_user_profile'));  
...
public function update_viewed_profile_post() {
     ...
     $log_user_profile = trim($this->post('logged_user_profile'));
     $profile = trim($this->post('another_profile_id'));
     $log_user_regid = trim($this->post('logged_user_tid'));
     $regid = trim($this->post('anotherprofile_tid'));
...
AND height <= '$height_to'";
			...
			if($education_field == 'Any' || $education_field_s == '' ){
			  ...
			  if($education_field1 == '""'){
			      ...
			      $ineducation_field = 'OR education_field IN ('.$education_field1.')';  
			  }
			}
			...
			if($highest_education == 'Any' || $highest_s == '' ){
			  ...
			  if($highest1 == '""'){
			      ...
			      $inhighest = 'OR highest_education IN ('.$highest1.')';  
			  }
			}
...
// 	$this->db->select("*");
//     $this->db->from('user_register');
//     $this->db->join('contact_info', 'user_register.profile = contact_info.reg_profil_id');
//     $this->db->join('education_work', 'user_register.profile = education_work.reg_profil_id');
//     $this->db->join('personal_habits', 'user_register.profile = personal_habits.reg_profil_id');
//     $this->db->where('user_register.status','1');
//     $this->db->where($marital_status);
...
//     $querymp= $this->db->get();
	...
	$querymp = $this->db->query("select user_register.id,user_register.profile, user_register.status,user_register.first_name,user_register.created_user,user_register.dob,user_register.height,user_register.caste,user_register.martial_status, user_register.verified,contact_info.perm_city,education_work.highest_education,education_work.education_field,education_work.occup ,personal_habits.diet
	 from user_register
        ...
        where user_register.status = '1' AND gender = '$show_gender' $marital_status $age $height $incaste $incity $inhighest $inoccup 
...
// 	$quecon = $this->db->query("SELECT *  FROM contact_info WHERE reg_profil_id = '$profile' $incity ");
//     foreach($quecon->result() as $rowcon)
// 	{ 
	    $perm_city = $rowrev->perm_city;
	
// 	$queedu = $this->db->query("SELECT *  FROM education_work WHERE reg_profil_id = '$profile'  $inhighest $inoccup
// 	 	 "); 
	 
//     foreach($queedu->result() as $rowedu)
// 	{ 
	    $highest_education = $rowrev->highest_education;
	    $education_field = $rowrev->education_field;
	    $occupw = $rowrev->occup;
...
public function photo_request_post() {
     ...
     $log_user_profile = trim($this->post('logged_user_profile'));
     $profile = trim($this->post('another_profile'));
...
//   }}
    ...
    $this->response([
  	   ...
  	   //=============================================================//
 public function profile_incomplete_post()
  {
       $log_user_profile =   $this->input->post('logged_user_profile');
        ...
        if($query1->num_rows() > 0){
         ...
         foreach ($query1->result() as $row1){
            $user_id = 	$row1->id;
          $log_user_weight = 	$row1->weight;
          $log_watsapp_num = 	$row1->watsapp_num;
         }
       } 
  ...
  if($log_user_rashi == '')
  {
    ...
    if($log_user_education_field == '' || $log_user_primary_edu == '')
  {
    $info5 =  1;
  }else{
       $info5 =  0;
    	   ...
    	   //=============================================================//
 public function first_step_profile_incomplete_post()
  {
       $log_user_profile =   $this->input->post('logged_user_profile');
        ...
        if($query1->num_rows() > 0){
         ...
         foreach ($query1->result() as $row1){
          ...
          $user_id = 	$row1->id;
          $mobile = 	$row1->mobile;
          $log_user_marital_status = 	$row1->martial_status;
          $log_caste = 	$row1->caste;
 ...
 //========================================================// 
  public function update_watsapp_num_post() {
      
      $mobile =   $this->input->post('mobile');
      $profile =   $this->input->post('profile');
      
      $this->db->set('watsapp_num', $mobile);
    $this->db->where('profile', $profile);
    $register =   $this->db->update('user_register');
    if($register){
    $this->response([
